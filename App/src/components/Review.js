...
export default function Review(props) {
    ...
    const GET_REVIEW_QUERY =gql`
        query{
            reviews(where:{product:{id:${props.product.id}}}){
                id
                review
                rate
                user{
        ...
        return (
            <View style={{padding:heightPercentageToDP(2),flexDirection:'row'}}>
                        <View>
                            ...
                            :<Image source={{uri:`${item.user.avatar.url}`}} style={{width:30,height:30,borderRadius:heightPercentageToDP(20),backgroundColor:'lightgrey'}} />}
                        </View>
