    ...
    return loading ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} ><ActivityIndicator color="lightblue" size='large' /></View> : (
        <View>
            ...
            {data.products.length == 0 ? <Empty />  :
                ...
                <FlatList
                    data={data.products}
                    renderItem={({ item, index }) => type == item.type || type== "All" ?  <ProductCard onPress={() => props.navigation.navigate("ProductDetail",{item:item})} key={`${index}${item.id}`} removeFromCartHandler={() =>{ dispatch(removeFromCart(item.id));
                        return ToastAndroid.show("Product removed to Cart !", 2000);}} product={item} />:null }
...
const ProductCard = ({ product,removeFromCartHandler }) => {
    ...
    return (<Pressable onPress={() => setvisible(!visible)}  >
        ...
        <TouchableOpacity onPress={addToCartHandle} style={{ position: 'absolute', right: 15, top: 25, }}>
       ...
       {!visible ? null : <View style={{margin:20,marginTop:7}}>
            ...
            <Text style={{color:'black',marginTop:10,fontSize:12}}>{product.description}</Text>
        </View> }
    </Pressable>)
}
